<template>
  <div class="home">
    <h1>Welcome to my Nightclub: {{this.myClub.getName()}}</h1>
      ...
      <el-col :span="18">
        <div style="background-color: #8ac7db;">
          ...
          <el-row :gutter="20" type="flex" justify="center">
              <AddDrink :inventory="this.myClub.getOverstock()" :overstock="[]" :addingToBar="false"></AddDrink>
              <EditStock :inventory="this.myClub.getOverstock()" :inventoryOwnerName="this.myClub.getName()"></EditStock>
              <el-button type="warning" @click="goShopping"  v-if="this.myClub.getOverstock().calculateStock().length > 0">
          ...
          <el-row :gutter="20" type="flex" justify="center" >
            <el-col :span="6">
              <div style="background-color: #DCDFE6;" class="grid-content bg-purple">
                <h3>Current Stock:</h3>
                <el-row type="flex" justify="center" v-for="inventoryItem in this.myClub.getOverstock().getItems()" :key="inventoryItem.getDrinkType()">
                  <el-col :span="2"/>
                  <el-col :span="10">
                    {{inventoryItem.getDrinkType().getName()}}
                  </el-col>
                ...
                <span v-if="this.myClub.getOverstock().getItems().length == 0">No Drinks in the system.</span>
                <br>
              </div>
            </el-col>
            <el-col :span="6"  >
              <div style="background-color: #C0C4CC;" class="grid-content bg-purple">
                <h3>Needs:</h3>
                <div class="">
                  <el-row type="flex" justify="center" v-for="inventoryItem in this.myClub.getOverstock().calculateStock()" :key="inventoryItem.getDrinkType()">
                    <el-col :span="2"/>
                    <el-col :span="10">
                      {{inventoryItem.getDrinkType().getName()}}
                    </el-col>
                    <div style="border-style: inset; padding-left: 4px; padding-right: 4px;">
                      <el-col :span="10">
                        {{inventoryItem.getRequiredStock()}}
                      </el-col>
                    </div>
                    <el-col :span="2"/>
                  </el-row>
                </div>
                <span v-if="this.myClub.getOverstock().calculateStock().length == 0">Fully Stocked</span>
    ...
    <br>
    <el-row :gutter="20">
      <el-col :span="6" v-for="bar in this.myClub.getBars()" :key="bar.getName()">
          ...
          <el-row type="flex" class="row-bg" justify="space-between">
            <el-col :span="8">
              <el-tooltip :disabled="tooltipDisabled" class="item" effect="dark" content="Click Button to Add a drink to the bar" placement="top">
                <AddDrink :inventory="bar.getInventory()" :drinkList="bar.getOverstock().getDrinkList()" :addingToBar="true"></AddDrink>
              </el-tooltip>
            </el-col>
            <el-col :span="8">
              <el-tooltip :disabled="tooltipDisabled" class="item" effect="dark" content="Changes Current and Required Stock" placement="top">
                <EditStock :inventory="bar.getInventory()" :inventoryOwnerName="bar.getName()"></EditStock>
              </el-tooltip>
            </el-col>
            <el-col :span="8">
              <el-tooltip :disabled="tooltipDisabled" class="item" effect="dark" content="Replenishes Stock from the Backroom" placement="top">
                <el-button v-if="bar.getInventory().calculateStock().length > 0" type="warning" @click="restockBar(bar)">Restock Bar</el-button>
              </el-tooltip>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12" style="height: match-parent">
              <div style="background-color: #DCDFE6;" class="grid-content bg-purple">
                <h3>Current Stock:</h3>
                <el-row type="flex" justify="center" v-for="inventoryItem in bar.getInventory().getItems()" :key="inventoryItem.getDrinkType()">
                  <el-col :span="2"/>
                  <el-col :span="10">
                    {{inventoryItem.getDrinkType().getName()}}
                  </el-col>
            ...
            <el-col :span="12" style="height: 100px">
              <div style="background-color: #C0C4CC;" class="grid-content bg-purple">
                <h3>Needs:</h3>
                <el-row type="flex" justify="center" v-for="inventoryItem in bar.getInventory().calculateStock() " :key="inventoryItem.getDrinkType()">
                  <el-col :span="2"/>
                  <el-col :span="10">
                    {{inventoryItem.getDrinkType().getName()}}
                  </el-col>
                ...
                <div v-if="bar.getInventory().calculateStock().length == 0">
                  Fully Stocked
                  <br>
                  <br>
                </div>
...
export default class Home extends Vue {
  ...
  public preloadValues(): void {
    ...
    for (const drink of this.preloadedDrinks) {
      ...
      this.myClub.getOverstock().addDrink(newDrink, 12, 3);
      i++;
    }

    const drinkList = this.myClub.getOverstock().getItems();
    i = 0;
    for (const barName of this.preloadedBars) {
      const newBar = new Bar(barName, new Inventory([]), this.myClub.getOverstock());
      newBar.getInventory().addDrink(drinkList[i].getDrinkType(), 1, i);
  ...
  public createBar(): void {
    const newBar = new Bar(this.newBarName, new Inventory([]), this.myClub.getOverstock());
    this.myClub.addBar(newBar);
  }
  ...
  public goShopping(): void {
    this.myClub.getOverstock().resetStock();
  }
